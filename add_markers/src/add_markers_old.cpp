...
{
public:
  std::vector<float> pick_up_point = {1.0, 1.0, 1.0};
  std::vector<float> drop_off_point = {2.5, 2.5, 1.0};
  float threshold = 0.05;
  bool reachedX, reachedY, reachedW, reachedPickup, reachedDropoff = false;
  bool checkDist(float a, float b, float thr);
  bool add_markers::targetReached(const nav_msgs::Odometry::ConstPtr &odom_msg, std::vector<float> &targetpose);
  void odomCallback(const nav_msgs::Odometry::ConstPtr &msg);
};

bool add_markers::checkDist(float a, float b, float thr)
{
  if (std::abs(a - b) < thr)
  {
    return true;
...
bool add_markers::targetReached(const nav_msgs::Odometry::ConstPtr &odom_msg, std::vector<float> &targetpose)
{
  ...
  reachedW = checkDist(odom_msg->pose.pose.orientation.w, targetpose[2], threshold);
  return (reachedX && reachedY && reachedW);
}
...
int main(int argc, char **argv)
{
  ...
  // call add_markers class
  add_markers myMarkers;

  ros::Subscriber odom_subscriber = n.subscribe("odom", 100, add_markers::odomCallback);
  ...
  while (ros::ok())
  {
    ...
    pickup_marker.pose.position.x = myMarkers.pick_up_point[0];
    pickup_marker.pose.position.y = myMarkers.pick_up_point[1];
    ...
    pickup_marker.pose.orientation.w = myMarkers.pick_up_point[2];
    ...
    // Set the pose of the marker.  This is a full 6DOF pose relative to the frame/time specified in the header
    dropoff_marker.pose.position.x = myMarkers.drop_off_point[0];
    dropoff_marker.pose.position.y = myMarkers.drop_off_point[1];
    ...
    dropoff_marker.pose.orientation.w = myMarkers.drop_off_point[2];
